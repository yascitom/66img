...
- 🧩 **大文件分片上传**：>10MB 自动走 OSS Multipart，逐片进度百分比；**断点续传**（进度存本机，中断/刷新/关页面后重选同一文件自动接着传）；分片失败自动重试 3 次并重新签名
...
2. **额外做一步：配置 Bucket 的 CORS**（教程里没有，但浏览器直传需要；**分片上传必须有 PUT 和 ETag**）：
   ...
   - 暴露 Header（ExposeHeaders）：`ETag`（⚠️ 缺了它浏览器读不到分片 ETag，无法合并）
