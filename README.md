...
- 🔒 密码登录门禁（UPLOAD_PASSWORD，≥10 位强制；验证通过换 7 天登录令牌，本机不存明文密码）
...
密码验证通过后，服务端会签发一个 **7 天有效的登录令牌**（HMAC-SHA256 签名，密钥为 OSS SK），前端此后只保存令牌、**不再保存明文密码**；7 天内打开自动解锁，过期后回到锁屏重新输入密码即可。
...
> 安全说明：登录门禁只是前端体验层，真正的拦截在函数里——`/api/sign`、`/api/multipart`、`/api/list`、`/api/delete` 都会校验身份（密码或 7 天登录令牌），不对直接 401；分片接口的 key 强制 `upweb/` 前缀白名单，且 init 会签发绑定 key/uploadId/声明大小/分片上限的会话令牌，合并前还会向 OSS 核验实际分片，无法越权操作桶内其他对象、也无法「声明小传大」。若未设置 `UPLOAD_PASSWORD` 或密码少于 10 位，所有接口将拒绝服务（除非显式设置 `ALLOW_ANONYMOUS_UPLOAD=true`）。

## 接口一览

| 接口 | 方法 | 说明 |
|---|---|---|
| `/api/sign` | POST | 小文件上传签名（PostObject Policy，**key 精确绑定**本次生成的对象键 + 防覆盖）；`{check:true}` 时为身份预检，**通过后签发 7 天登录令牌** |
| `/api/multipart` | POST | 大文件分片上传：`action=init` 初始化（返回 uploadId+key+partSize+**会话令牌**）；`action=part` 为单个分片签发 1 小时有效的预签名 URL（校验会话令牌，分片号不得超声明上限）；`action=complete` 合并分片（先 ListParts 核验实际分片数/总字节，超声明值自动 Abort 清理）；`action=abort` 清理残留分片。全程密钥不出服务端 |
...
- AccessKey 只存于服务端环境变量，前端不可见；HMAC 令牌签名密钥复用 OSS SK，不新增环境变量
...
- 前端**不保存明文密码**：登录成功换取 7 天 HMAC 登录令牌（过期自动回锁屏），本机只存令牌
- 密码校验为 SHA-256 哈希比对 + 失败统一延迟 0.4s，拖慢在线暴力破解；仍请务必使用**强密码（≥10 位，建议更长）**
- 上传签名 Policy 的 key **精确绑定**本次生成的对象键（`eq` 条件）+ `x-oss-forbid-overwrite` 防覆盖，拿到签名也无法改写其他路径或覆盖已有文件
- 分片上传有**会话令牌绑定**：init 时签发、绑定 key/uploadId/声明大小/分片数上限；part 分片号超限直接拒绝；complete 前向 OSS ListParts 核验实际分片数与总字节，超出声明值自动 Abort 清理会话——堵住"声明小传大"绕过大小限制
